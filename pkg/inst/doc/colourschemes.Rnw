...
The rgb and hsv schemes are specialised cases of the ``f3'' scheme. This scheme
takes a function as parameter in its constructor that takes three equal-length vector 
arguments and returns colour values. The existing \texttt{rgb} and \texttt{hsv} functions
in R fit this pattern, but you can write your own. The constructor also takes the
\texttt{equalScale} argument and needs a label string to identify your scheme.
