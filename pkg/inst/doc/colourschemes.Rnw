...
This function takes whatever data and options are needed. Then you need to make sure
these options are evaluated in order to fix their values. Use the \texttt{force} function
for this:
...
See how this function takes the data and also has the values of \texttt{a}, \texttt{b},
and \texttt{c} within it.

Then we need to make sure we return the function with a new class, and a 'type' string
which is a short description:
